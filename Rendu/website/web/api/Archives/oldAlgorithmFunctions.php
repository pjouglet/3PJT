...
function getRecursiveJourneys($startTime, $startStationId, $endStationId, $lastConnections, $excludedStations, &$connections, &$journeyConnectionArray)
{
    ...
    foreach ($connections as $connection)
    {
        if ($connection->startStationId == $startStationId && $connection->startTime >= $startTime && !in_array($connection->arrivalStationId, $excludedStations))
        {
            ...
            getRecursiveJourneys($connection->arrivalTime, $connection->arrivalStationId, $endStationId, $lastConnectionTmp, $excludedStationTmp, $connexions, $journeyConnectionArray);
        }
    }
}
...
function findCheapestJourneyInZone($startStation, $arrivalStation, $startTime) {
    ...
    foreach ($timetable as $connection) {
        if ($connection->startStationId == $startStation->id) {
            ...
            getRecursiveJourneys($connection->arrivalTime, $connection->arrivalStationId, $arrivalStation->id, $lastConnectionTmp, $excludedStationTmp, $timetable, $journeyConnectionArray);
        }
    }
